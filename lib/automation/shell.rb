...
module Automation
  ...
  module Shell
    ...
    def self.capture(*cmd, chdir: nil, env: {})
      opts = chdir ? { chdir: chdir } : {}
      Open3.capture3(env.transform_keys(&:to_s), *cmd, **opts)
    end
    ...
    def self.run_network(*cmd, chdir: nil, allow_failure: false, env: {})
      ATTEMPTS.times do |i|
        out, err, status = capture(*cmd, chdir: chdir, env: env)
        return out if status.success?
  ...
  module Gitlab
    ...
    def self.api(path, method: 'GET', form: {}, allow_failure: false)
      ...
      out = Shell.run_network(*cmd, allow_failure: allow_failure, env: bot_env)
